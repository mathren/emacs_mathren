  ...
  (condition-case err
      (let ((local-bib (citar-bibtool-get-local-bib-file))
        ...
        (when (and (file-exists-p master-bib) key)
          ...
          (if (citar-bibtool-entry-exists-in-local-bib-p key local-bib)
              (let ((choice (completing-read
                             (format "Entry '%s' already exists in local bib. Choose action: " key)
                             '("use-existing" "rename-existing")
                             nil t)))
                (when (string= choice "rename-existing")
                  (let ((new-key (read-string "New key for existing entry: " key)))
                    ...
                    (with-temp-buffer
                      ...
                      (write-file local-bib)))))
            ;; Entry doesn't exist, proceed with normal copy
...
(defun citar-bibtool-update-citar-bibliography ()
  "Update citar-bibliography to include local bib file."
  (condition-case nil
      (let ((local-bib (citar-bibtool-get-local-bib-file)))
        (when (file-exists-p local-bib)
          (setq citar-bibliography local-bib)))
    (error (message "No local bibtex file found!"))))
...
(defun citar-bibtool-setup-local-workflow ()
  ...
  (interactive)
  ;; Update citar-bibliography with local bib file
  (citar-bibtool-update-citar-bibliography)
...
Checks for duplicates and suggests existing keys when found."
  ...
  (let* ((query (read-string "Search NASA/ADS: "))
    ...
    (when final-bibtex
      (with-temp-buffer
        (when (file-exists-p local-bib-path)
          (insert-file-contents local-bib-path))
        (goto-char (point-max))
        (unless (bobp) (insert "\n\n"))
        (insert final-bibtex)
...
(defun citar-bibtool-insert-tex-bib (citekeys)
  ...
  (let* ((local-bib-file (citar-bibtool-get-local-bib-file))
    ...
    (dolist (citekey citekeys)
      (if (citar-bibtool-entry-exists-in-local-bib-p citekey local-bib-path)
          (let ((choice (completing-read
                         (format "Entry '%s' already exists in local bib. Choose action: " citekey)
                         '("use-existing" "rename-existing")
                         nil t)))
            (when (string= choice "rename-existing")
              (let ((new-key (read-string "New key for existing entry: " citekey)))
                ;; Replace key in local bib file
                (with-temp-buffer
                  (insert-file-contents local-bib-path)
                  (goto-char (point-min))
  ...
  (condition-case err
      (let* ((local-bib-base (citar-bibtool-get-local-bib-file))
        ...
        (let ((full-local-bib-path (expand-file-name local-bib)))
          ...
          (setq citar-bibliography full-local-bib-path) ;; Changed to string
