...
(use-package citar
  ...
  :config
  ;; Add the directory to load path and require the module
  (add-to-list 'load-path "~/.emacs.d/emacs_tools/citar-bibtool/")
  (require 'citar-bibtool)
  ...
  ;; Set up keybindings for latex-mode
  (add-hook 'latex-mode-hook
	    (lambda ()
	      (local-set-key (kbd "C-c i l") #'citar-bibtool-insert-citation-from-local-bib)
	      (local-set-key (kbd "C-c i m") #'citar-bibtool-insert-tex-bib)
	      (local-set-key (kbd "C-c i a") #'ads-search-and-insert-citation)
	      (local-set-key (kbd "C-c i R") #'citar-bibtool-sync-all-citations-to-local-bib)
	      (local-set-key (kbd "C-c i C") #'citar-bibtool-insert-citation-with-local-copy)))

  ;; Set up keybindings for LaTeX-mode (AUCTeX)
  (add-hook 'LaTeX-mode-hook
	    (lambda ()
	      (local-set-key (kbd "C-c i l") #'citar-bibtool-insert-citation-from-local-bib)
	      (local-set-key (kbd "C-c i m") #'citar-bibtool-insert-tex-bib)
