...
(use-package! elfeed
  :config
  (setq elfeed-feeds
	'("https://export.arxiv.org/rss/astro-ph/new") ;; RSS working - no authors
	;; '("http://export.arxiv.org/api/query?search_query=cat:astroph&start=0&max_results=120&sortBy=submittedDate&sortOrder=descending")
	)

  (add-hook! 'elfeed-search-mode-hook 'elfeed-update)
    (defun mr/concatenate-authors (authors-list)
    ...
    (defun mr/elfeed-mark-all-as-read ()
      "mark all as read before updating database
       work-around to see only news
      (add-hook 'elfeed-db-update-hook #'mr/elfeed-mark-all-as-read)
      "
      (interactive)
    ...
    (setq elfeed-search-print-entry-function #'mr/search-print-fn)
    )
