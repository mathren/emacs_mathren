...
** Tramp for remote connections

  Fix location of perl on one specific server
...
**** Latex in markdown ein cells
     ...
     The function below will execute notebook cells, or, if they are a =markdown=
     cell, it will try rendering the =tex= expression at point:

#+BEGIN_SRC emacs-lisp  :tangle ~/Documents/dotfiles/emacs_mathren/dot-emacs.d/init.el
   (defun mr/ein:worksheet-execute-cell (ws cell &optional batch)
...
#+END_SRC
    ...
    I bind it to =C-c C-c= to supersede =ein:worksheet-execute-cell=.
