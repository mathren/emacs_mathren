...
[[https://github.com/emacs-citar/citar][Citar]] has more features than the built in =reftex=. The
...
(see [[https://github.com/mathren/binary_script/blob/main/merge_bibtex.sh][this bash script]]). Requires the file in
[[./emacs_tools/citar-bibtool.el]].

#+BEGIN_SRC emacs-lisp
    (use-package citar
    :ensure t
    :bind (("C-c c" . citar-insert-citation)
    ...
    (with-eval-after-load 'latex
      (define-key latex-mode-map (kbd "C-c b") #'citar-bibtool-insert-citation-with-local-copy)
      (define-key latex-mode-map (kbd "C-c B") #'citar-bibtool-sync-all-citations-to-local-bib))
