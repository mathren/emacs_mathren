...
#+BEGIN_SRC emacs-lisp
  ...
  (defun mr/elfeed-mark-all-as-read ()
    "mark all as read before updating database work-around to see only news"
...
I also redefine the function =elfeed-show-entry= to enforce horizontal
splitting of the window when looking at an elfeed entry (i.e., an
abstract on arXiv)
...
*** TODO render latex properly in elfeed entries
