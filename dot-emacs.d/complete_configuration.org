...
** Avoid creating tons of hidden files
   ...
   May conflict with saving remote jupyter notebooks with =ein=, see
   below.
...
** Language server protocol
  ...
  I am using [][eglot] for language server protocol needs, mostly in
  Fortran 90 and python (the latter requires to =pip install
  python-lsp-server=)
