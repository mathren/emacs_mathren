...
=~/Documents/dotfiles/emacs_mathren/dot-emacs.d/minimal.el= using (=C-c
C-v t= which is bound to =org-babel-tangle=). This file is then managed
using [[https://www.gnu.org/software/stow/][gnu stow]] and hyperlinked to =~/.emacs.d/minimal.el.=
...
Disable bidirectional text scanning (I don't use scripts that are
written left-to-right very often):
...
Auto select help buffer if opened:
...
Avoid syntax highlighting as you type

#+BEGIN_SRC  emacs-lisp :tangle ~/Documents/dotfiles/emacs_mathren/dot-emacs.d/minimal.el
(setq redisplay-skip-fontification-on-input t)
#+END_SRC

* Create parent directory if not existing for new files
A common situation: I am in folder =/foo= and I want to create a
subfolder =bar/= and inside it a text file =file-name=. I do =C-x C-f= to
launch =find-file=, type =bar/file-name= which gets appended to the local
path =/foo=, and emacs opens a new buffer. But because =bar= doesn't
exist, this leads to many errors.

This hook allows to create the parent directory if non existing:
#+BEGIN_SRC emacs-lisp :tangle ~/Documents/dotfiles/emacs_mathren/dot-emacs.d/minimal.el
...
Save clipboard content from programs other than emacs in the kill ring

#+BEGIN_SRC emacs-lisp :tangle ~/Documents/dotfiles/emacs_mathren/dot-emacs.d/minimal.el
(setq save-interprogram-paste-before-kill t)
#+END_SRC

Prevent duplicates in kill ring
...
When creating a new buffer (e.g., with =C-x 2= or =C-x 3=) I want to resize
all buffers
...
Make focusing on current buffer with =C-x 1= reversible:
